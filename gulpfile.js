...
gulp.task('prod', ['umdBuild'], function() {
  ...
  gulp.src(['./src/**/*.js', './src/*js'])
    ...
    .pipe(gulp.dest('./lib'));
  // build the css
  gulp.src('./css/oton-bootstrap-table.css')
    .pipe(concatCss("./oton-bootstrap-table.min.css"))
    .pipe(cssmin())
    .pipe(gulp.dest('./dist'));
  gulp.src(['./css/oton-bootstrap-table.css',
    './node_modules/react-s-alert/dist/s-alert-default.css',
    './node_modules/react-s-alert/dist/s-alert-css-effects/scale.css'])
    .pipe(concatCss('./oton-bootstrap-table-all.min.css'))
    .pipe(cssmin())
    .pipe(gulp.dest('./dist'));
});
